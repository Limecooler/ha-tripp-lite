...
async def test_service_validation_and_error_branches(
    ...
    assert (
        integration._load_from_unique_id(runtime_data, "entry", "entry_1_load_main_switch")
        is not None
    )
    assert (
        integration._load_from_unique_id(runtime_data, "entry", "entry_1_load_absent_switch")
    ...
    assert (
        integration._variable_from_unique_id(
            runtime_data, "entry", "entry_1_variable_4_number", "number"
        )
        is not None
    )
    assert (
        integration._variable_from_unique_id(
            runtime_data, "entry", "entry_1_variable_missing_number", "number"
        )
        is None
    )
